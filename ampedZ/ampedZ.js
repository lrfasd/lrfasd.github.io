...
function ReadBuffer() {
	if (Name.startWith("ampedZ") && Name.endWith(".zar")) {
		Data = new Zar(Buffer);
		if (Data.IsValid) {
...
function Initialize() {
  ...
  var LoadUrlParameter = (function() {
	...
	var LoadFileInfo = (function() {
		var list = {"file": ["ampedZ.zar", "ampedZ_r.zar"], "length": ["", ""], "intro": ["设定包", "设定包 + 资源包"]};
		...
		for (var i = 0; i < list[n].length; i++) {
			...
			for (var n in list) {
				...
				c.innerHTML = list[n][i];
			}
		}
		var tf = t.createTFoot();
		document.getElementById("divInfo").appendChild(t);
		document.getElementById("divInfo").style.display = "block";
	})();
	var LoadFileLength = function() {
		var heads = document.getElementById("divInfo").childNodes[0].childNodes[0].childNodes[0];
		var rows = document.getElementById("divInfo").childNodes[0].childNodes[1].childNodes;
		var getHeadIndex = function(n) {
			for (var i = 0; i < heads.childNodes.length; i++) {
				if (n == heads.childNodes[i].innerHTML) {
					return i;
				}
			}
			return -1;
		};
		var getFileLength = function(n) {
			var r = new XMLHttpRequest();
			r.onreadystatechange = function() {
				switch(r.readyState) {
				case 2:
					return r.getResponseHeader("Content-Length");
					break;
				}
			};
			r.open("GET", n);
		...
		var nFile = getHeadIndex("file");
		var nLength = getHeadIndex("length");
		if ((nFile != -1) && (nLength != -1)) {
			for (var i = 0; i < rows.length; i++) {
				rows[i].childNodes[nLength].innerHTML = getFileLength(rows[i].childNodes[nFile].innerHTML);
			}
		}
	};
	if (Runtime != "file") {
