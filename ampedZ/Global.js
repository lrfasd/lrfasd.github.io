...
window.lsLocal = function(t) {
	var img = new Image();
	img.src = "";
	return img.src.startsWith("file");
};
...
Uint8Array.prototype.getHex = function() {
	...
	var e = s + l;
	for (var i = s; i < e; i++) {
		r.push((this.buffer[i] || this[i]).toString(16).padStart(2, "0"));
	}
	r = r.join(c);
	if (b) {
		r = r.toUpperCase();
...
Uint8Array.prototype.extractFileList = function() {
  var buffer = this;
	var pl = [];
	var fl = [];
	var p = 0;
...
Date.prototype.format = function() {
	var m = (arguments.length > 0 ? arguments[0] : "datetime");
	var r = [];
	if (m.indexOf("date") != -1) {
		r.push(this.getFullYear().toString().padStart(4, "0") + "." + (this.getMonth() + 1).toString().padStart(2, "0") + "." + this.getDate().toString().padStart(2, "0"));
	}
	if (m.indexOf("time") != -1) {
		r.push(this.getHours().toString().padStart(2, "0") + ":" + this.getMinutes().toString().padStart(2, "0") + ":" + this.getSeconds().toString().padStart(2, "0"));
	}
	return r.join(" ");
};
