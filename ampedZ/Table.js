...
function Zar(buffer) {
	var isValid = false;
	var Catagory = ["Style", "Accessory", "SubAccessory", "Item", "Head", "Body", "Coupon", "Card", "CardEffect", "Furniture", "Emblem", "Emotion", "Emoticon"];
	var File = {};
	var Data = {};
  var Icon = {};
  var Table = {};
	...
	var fromBuffer = function() {
		...
		Data = (function() {
			...
			if (File["setting.ser"]) {
				...
				r["Accessory"] = (function() {
					...
					if (ser[k]) {
						for (var n in ser[k]) {
							var v = ser[k][n];
							if (v["id"]) {
								d.push({"setidx": v["setidx"], "id": v["id"], "name": v["name"], "desc": v["desc"], "sex": v["sex"], "multiaccs": v["multiaccs"], "editable": v["editable"], "pcvan": v["pcvan"], "useablestyles": ""});
							}
						}
						d = d.sort(s);
					}
				...
				r["SubAccessory"] = (function() {
					...
					if (ser[k]) {
						for (var n in ser[k]) {
							var v = ser[k][n];
							if (v["id"]) {
								d.push({"setidx": v["setidx"], "id": v["id"], "name": v["name"], "desc": v["desc"], "sex": v["sex"], "multiaccs": v["multiaccs"], "editable": v["editable"], "pcvan": v["pcvan"]});
							}
						}
						d = d.sort(s);
					}
				...
				r["Item"] = (function() {
					...
					if (ser[k]) {
						for (var n in ser[k]) {
							var v = ser[k][n];
							if (v["id"]) {
								d.push({"id": v["id"], "name": v["name"], "desc": v["desc"], "sex": v["sex"], "class": v["cls"].join("")});
							}
						}
					}
					return d;
				...
				r["Head"] = (function() {
					...
					if (ser[k]) {
						for (var n in ser[k]) {
							var v = ser[k][n];
							if (v["id"]) {
								d.push({"id": v["id"], "name": v["name"], "sex": v["sex"], "init": v["init"]});
							}
						}
					}
					return d;
				...
				r["Body"] = (function() {
					...
					if (ser[k]) {
						for (var n in ser[k]) {
							var v = ser[k][n];
							if (v["id"]) {
								d.push({"id": v["id"], "name": v["name"], "init": v["init"]});
							}
						}
					}
					return d;
				...
				r["Coupon"] = (function() {
					...
					if (ser[k]) {
						for (var n in ser[k]) {
							var v = ser[k][n];
							if (v["id"]) {
								d.push({"id": v["id"], "name": v["name"], "category": v["category"], "type": v["type"], "value": v["value"], "present": v["present"], "period": v["period"], "periodstr": v["periodstr"]});
							}
						}
					}
					return d;
				...
				r["Card"] = (function() {
					...
					if (ser[k]) {
						for (var n in ser[k]) {
							var v = ser[k][n];
							if (v["id"]) {
								d.push({"id": v["id"], "name": v["name"], "desc": v["desc"], "elements": br(v["elements"].map(function(a){return a["effectid"];})), "priceGrade": v["priceGrade"]});
							}
						}
					}
					return d;
				})();
				r["CardEffect"] = (function() {
					var d = [];
				...
				r["Furniture"] = (function() {
					...
					if (ser[k]) {
						for (var n in ser[k]) {
							var v = ser[k][n];
							if (v["id"]) {
								d.push({"id": v["id"], "name": v["name"], "desc": v["desc"], "type": v["type"]});
							}
						}
					}
					return d;
					...
					if (ser[k]) {
						for (var n in ser[k]) {
							...
							if (v["id"]) {
								...
								d.push({"id": v["id"], "name": v["name"], "desc": v["desc"], "msg": v["msg"], "pts": getOdds("pts"), "gm": getOdds("gm"), "sex": v["sex"], "iconmax": v["iconmax"], "conditions": br(v["conditions"]), "get_type": br(v["get_type"])});
							}
						}
					}
					return d;
				...
				r["Emoticon"] = (function() {
					...
					if (ser[k]) {
						for (var n in ser[k]) {
							var v = ser[k][n];
							if (v["id"]) {
								d.push({"id": v["id"], "name": v["name"], "grade": v["grade"], "time": v["time"], "loop": v["loop"]});
							}
						}
					}
					return d;
				...
				'URL.createObjectURL(new Blob([d], {"type": "image/bmp"}))'
